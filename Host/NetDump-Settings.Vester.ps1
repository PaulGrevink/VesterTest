...
[ScriptBlock]$Actual = {
	@(
	        ...
	        # As the config can only have one value per item.
            # All DCR hosts have IP 10.44.100.224 and
            # All WPR hosts have IP 10.44.100.225
            # Code will check the actual IP against the desired values.
            # If correct, it will return the desired value in the config file and [+]
            # If false, it will return the actual value and [-]
            $realIP = (Get-EsxCli -v2 -VMhost $Object).system.coredump.network.get.Invoke().NetworkServerIP
            if ($realIP -eq "10.44.100.224" -and $Object -match 'dcr') {
                "10.44.100.224"
            } elseif ($realIP -eq "10.44.100.225" -and $Object -match 'wpr') {
                "10.44.100.224"
            } else {
                $realIP
            } 
